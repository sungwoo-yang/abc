...
Robot::Robot(Math::vec2 start_position, Cat* cat, double left, double right) : CS230::GameObject(start_position), target_cat(cat), left_boundary(left), right_boundary(right), speed(walking_speed)
{
    AddGOComponent(new CS230::Sprite("Assets/images/Robot.spt", this));
...
void Robot::State_Walking::CheckExit(GameObject* object)
{
    ...
    bool   facing_cat   = ((cat_x < robot_x && robot->GetScale().x < 0) || (cat_x > robot_x && robot->GetScale().x > 0));
